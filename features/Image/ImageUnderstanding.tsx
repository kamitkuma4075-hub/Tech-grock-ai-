...
const ImageUnderstanding: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!prompt.trim() || !imageFile || isLoading || !ai) return;
    ...
    try {
        ...
        const result = await ai.models.generateContent({
            ...
            config: {
                systemInstruction: "You are a professional AI assistant specializing in image analysis. Provide a detailed, professional, and accurate analysis based on the user's query about the image."
            }
        });
        ...
        <div className="flex flex-col space-y-4">
            {!imageFile ? (
                 <Uploader
                    ...
                    description="PNG, JPG, GIF, WEBP up to 10MB"
                 />
            ) : (
                <div className="relative">
                    <img src={imageUrl!} alt="User upload" className="w-full rounded-lg shadow-md" />
                    <button
                ...
                {isLoading && (
                    <div className="flex items-center justify-center h-full">
                        <div className="text-center">
                            <Spinner />
                            <p className="mt-4 text-gray-400">Gemini is analyzing the image...</p>
                        </div>
                    </div>
                )}
